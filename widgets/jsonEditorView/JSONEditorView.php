<?php
/**
 * JSONEditorView class file.
 *
 * @author Dariusz Górecki <darek.krk@gmail.com>
 * @link http://www.cleverit.com.pl
 * @copyright Copyright &copy; 2008-2010 CleverIT - Dariusz Górecki
 ...
 *
 * To use set {@link data} to the JSON string
 * @author Dariusz Górecki <darek.krk@gmail.com>
 * @version 1.0.1
 */
class JSONEditorView extends CWidget
{
	/**
	...
	/**
	 * @var string label for submit button, default: 'Save'
	 */
	...
	public function init()
	{
		...
		if(!isset($this->htmlOptions['style']))
			$this->htmlOptions['style'] = 'border:solid 2px #88F; width:100%;';
		...
		$cs->registerScript(
			'JSONEditorView-submit#'.$id,
			"
				jQuery(\"#{$id}-form\").submit(function(){
					var obj = jQuery(\"#{$id}\");

					obj.jsoneditor('input');
					obj.children('textarea').attr('name', '{$id}-json');
				});
			"
		);
	}
	...
	public function run()
	{
		...
		echo CHtml::submitButton($this->submitButtonLabel, array(
			'name'=>$this->htmlOptions['id'].'-save'
		));
